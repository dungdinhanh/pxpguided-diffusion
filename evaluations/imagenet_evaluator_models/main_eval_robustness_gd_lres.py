...
from evaluations.imagenet_evaluator_models.data_loader import data_loader, eval_loader, val_imagenet_loader_lowres
...
def main(local_rank):
    ...
    if args.arch == 'alexnet':
        ...
        if args.img_size == 64:
            model = densenet121lr(pretrained=pretrained)
        ...
        if args.img_size == 64:
            model = densenet201lr(pretrained=pretrained)
            conv0 = nn.Conv2d(3, 64, kernel_size=7, stride=1, padding=3, bias=False)
            n = conv0.kernel_size[0] * conv0.kernel_size[1] * conv0.out_channels
            conv0.weight.data.normal_(0, math.sqrt(2. / n))
            model.features.conv0 = conv0
        ...
        if args.img_size == 64:
            model = densenet161lr(pretrained=pretrained)
        ...
        if args.img_size == 64:
            # Change first layer
            conv1 = nn.Conv2d(3, 64, kernel_size=7, stride=1, padding=1, bias=False)
    ...
    # Data loading
    mini = args.local
    val_loader = val_imagenet_loader_lowres(args.batch_size, args.workers, args.pin_memory, mini, diff, args.img_size)
